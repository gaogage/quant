...
pub fn TasksPage() -> Element {
    ...
    rsx! {
        div { class: "p-6 max-w-6xl mx-auto",
            ...
            div { class: "mb-4 p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 flex items-center justify-between",
...
fn TaskRow(data: Value) -> Element {
    ...
    rsx! {
        div { class: "bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 flex items-center justify-between",
            div { span { class: "text-gray-900 dark:text-white text-sm font-medium", "{name}" } div { class: "text-xs text-gray-500 dark:text-gray-400 mt-1", "Cron: {cron} · 上次: {last_run} · 次数: {count}" } }
            if enabled {
                span { class: "text-xs px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400", "已启用" }
            } else {
                span { class: "text-xs px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400", "已禁用" }
            }
        }
    }
}
