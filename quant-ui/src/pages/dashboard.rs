...
pub fn DashboardContent() -> Element {
    ...
    rsx! {
        div { class: "p-6 max-w-6xl mx-auto",
            // 欢迎区
            div { class: "mb-8",
                h1 { class: "text-3xl font-bold text-white mb-2", "你好, {display_name}" }
            ...
            div { class: "grid grid-cols-3 gap-4 mb-8",
                div { class: "bg-gray-900 rounded-xl border border-gray-800 p-5",
                    div { class: "text-3xl font-bold text-blue-400", "{accounts.read().len()}" }
                    div { class: "text-sm text-gray-500 mt-1", "投资账号" }
                }
                div { class: "bg-gray-900 rounded-xl border border-gray-800 p-5",
                    div { class: "text-3xl font-bold text-green-400", "{strategies.read().len()}" }
                    div { class: "text-sm text-gray-500 mt-1", "可用策略" }
                }
                div { class: "bg-gray-900 rounded-xl border border-gray-800 p-5",
                    div { class: "text-3xl font-bold text-yellow-400",
                        if user.as_ref().map(|u| u.role == "admin").unwrap_or(false) { "管理员" } else { "用户" }
                    }
                    div { class: "text-sm text-gray-500 mt-1", "权限级别" }
                }
            }
            ...
            div { class: "mb-6",
                div { class: "flex items-center justify-between mb-3",
                    h2 { class: "text-lg font-semibold text-white", "投资账号" }
                    button {
                        class: "text-sm px-3 py-1.5 bg-blue-600 hover:bg-blue-500 rounded-lg transition",
                ...
                if accounts.read().is_empty() {
                    ...
                    for acc in accounts.read().iter().take(3) {
            ...
            div {
                div { class: "flex items-center justify-between mb-3",
                    h2 { class: "text-lg font-semibold text-white", "交易策略" }
                    button {
                        class: "text-sm px-3 py-1.5 bg-blue-600 hover:bg-blue-500 rounded-lg transition",
                ...
                if strategies.read().is_empty() {
                    ...
                    div { class: "grid grid-cols-2 gap-3",
                        for strat in strategies.read().iter().take(4) {
                            {
                                ...
                                rsx! {
                                    div { class: "bg-gray-900 rounded-xl border border-gray-800 p-4",
                                        div { class: "font-medium text-white text-sm", "{name}" }
                                        div { class: "text-xs text-gray-500 mt-1", "{owner}" }
                                        span { class: "inline-block mt-2 text-xs px-2 py-0.5 rounded-full bg-gray-800 text-gray-400",
                                            "{status}"
                                        }
                                    }
                                }
...
fn AccountCard(data: Value) -> Element {
    ...
    rsx! {
        div { class: "bg-gray-900 rounded-xl border border-gray-800 p-4 flex items-center justify-between",
            div {
                div { class: "font-medium text-white text-sm", "{name}" }
                div { class: "text-xs text-gray-500 mt-1", "{acc_type} · {signal}" }
            }
            div { class: "text-right",
                div { class: "text-sm text-gray-300", "¥{cap}" }
                span { class: "text-xs px-2 py-0.5 rounded-full bg-gray-800 text-gray-400", "{status}" }
            }
        }
    }
}
