...
base.SEARCH_SPACE = {
    "combo_name": {"type": "choice", "values": ["full_eq_16f", "full_icir_16f", "full_icir_16f_v2", "full_icir_16f_v3"]},
...
def run_window(idx, start_date, end_date):
    ...
    print(f"[{idx}/{len(base.WINDOWS)}] created {task_id} {start_date}-{end_date}", flush=True)
...
def main():
    ...
    for idx, (start_date, end_date) in enumerate(base.WINDOWS, start=1):
        if idx in done:
            continue
        results.append(run_window(idx, start_date, end_date))
