...
pub fn compute_ic(
    ...
    let rank_ic_ir = if std_ic > 1e-12 {
        mean_rank_ic / std_ic
    } else {
        0.0
    };
...
mod tests {
    ...
    fn test_compute_ic_basic() {
        ...
        fr.insert(
            ("A".into(), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap()),
            0.01,
        );
        fr.insert(
            ("B".into(), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap()),
            0.02,
        );
        fr.insert(
            ("C".into(), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap()),
            0.03,
        );
        fr.insert(
            ("D".into(), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap()),
            0.04,
        );
        fr.insert(
            ("E".into(), NaiveDate::from_ymd_opt(2024, 1, 1).unwrap()),
            0.05,
        );
