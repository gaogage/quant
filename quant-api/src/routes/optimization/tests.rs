...
fn alpha_source_research_diagnostics_options_are_explicit_and_bounded() {
    ...
    assert!(!alpha_source_research_diagnostics_options(&cheap_default).include_research_metrics);
}
...
fn oos_train_execution_defaults_to_shared_cache_even_when_concurrency_requested() {
    ...
    let policy =
        resolve_oos_train_execution_policy(&req, &LocalResourcePlan::for_machine(10, 32)).unwrap();
...
fn phase7_oos_profile_comparison_plan_defaults_to_ec_eu_ev_with_auto_cache() {
    ...
    let plan = build_phase7_oos_profile_comparison_plan(&req, None, None).expect("comparison plan");
...
fn phase7_oos_profile_comparison_launch_requests_are_bounded_background_smokes() {
    ...
    let requests =
        build_phase7_oos_profile_comparison_launch_requests(&req, None, None).expect("launch reqs");
...
fn train_fill_ratio_gate_allows_intentional_cash_when_target_is_filled() {
    ...
    assert!(gates
        .iter()
        .any(|gate| gate["gate"] == "train_final_unfilled_target_gap" && gate["passed"] == true));
    assert!(gates
        .iter()
        .any(|gate| gate["gate"] == "train_final_execution_fill_ratio" && gate["passed"] == true));
    assert!(!gates
        .iter()
        .any(|gate| gate["gate"] == "train_final_cash_weight"));
}
...
fn phase7_layered_request_accepts_regime_conditioned_valuation_guard_profile() {
    ...
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["event_gate_profile"] == "valuation_exclude_bottom35_stress_only"
            && trial.parameters["event_gate_active_regimes"] == json!(["bear", "high_volatility"])
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
...
fn phase7_layered_request_accepts_regime_alpha_sleeve_allocation_profile() {
    ...
    assert_eq!(bundle.plan.planned_trials, 4);
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["market_regime"] == "quality_regime_alpha_portfolio_sleeve_value_10pct_v1"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["market_regime"] == "quality_regime_alpha_portfolio_sleeve_value_15pct_v1"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["market_regime"] == "quality_regime_alpha_portfolio_sleeve_blend_10pct_v1"
    }));
}
...
fn phase7_layered_request_accepts_current_anchor_risk_shape_profile() {
    ...
    assert!(bundle
        ...
        .trials
        .iter()
        .any(|trial| { trial.parameters["portfolio_volatility_control"] == "vol120_16_50_100" }));
    assert!(bundle.plan.trials.iter().any(|trial| {
...
fn phase7_layered_request_accepts_position_sharpe_return_bridge_profile() {
    ...
    assert!(bundle
        ...
        .trials
        .iter()
        .all(|trial| { trial.parameters["event_gate_profile"] == "valuation_exclude_bottom45" }));
}
...
fn phase7_layered_request_accepts_correlation_frontier_sharpe_return_profile() {
    ...
    assert_eq!(bundle.plan.planned_trials, 12);
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["position_shape_profile"] == "maxpos16_corr705_router_v4_vol120_16_50_100"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["position_shape_profile"] == "maxpos165_corr72_router_v4_vol120_17_52_100"
    }));
    assert!(bundle.plan.trials.iter().all(|trial| {
        trial.parameters["market_regime"] == "quality_event_window_return_sharpe_router_v4"
    }));
...
fn phase7_layered_request_accepts_correlation_threshold_sharpe_return_profile() {
    ...
    assert_eq!(bundle.plan.planned_trials, 12);
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["position_shape_profile"] == "maxpos16_corr706_router_v4_vol120_16_50_100"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["position_shape_profile"] == "maxpos165_corr709_router_v4_vol120_17_52_100"
    }));
    assert!(bundle.plan.trials.iter().all(|trial| {
        trial.parameters["market_regime"] == "quality_event_window_return_sharpe_router_v4"
    }));
...
fn phase7_layered_request_accepts_portfolio_sharpe_control_profile() {
    ...
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["market_regime"] == "quality_state_sharpe_bridge_router_v2"
            && trial.parameters["portfolio_sharpe_control"] == "bridge_roll_sharpe180_050_neg10_60"
    }));
}
...
fn phase7_layered_request_accepts_prediction_confirmed_sharpe_bridge_profile() {
    ...
    assert!(bundle.plan.trials.iter().all(|trial| {
        trial.parameters["signal_source"] == "factor_combo"
            && trial.parameters["combo_name"] == "phase7_financial_quality_v1"
            && trial.parameters["prediction_set_id"] == "pred-p7-wf-wide-qgvrel-v1-201602-202605"
            && trial.parameters["risk_contribution_control"] == "soft_single_name_20pct_v1"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["market_regime"] == "quality_mixed_orthogonal_risk_memory_router_v3"
...
fn phase7_layered_request_accepts_execution_rolling_carry_profile() {
    ...
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["execution_carry_policy"] == "roll_forward_v1"
            && trial.parameters["execution_rules"]["execution_carry_policy"] == "roll_forward_v1"
    }));
}
...
fn phase7_layered_request_accepts_pit_capacity_ranking_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_er"));
    assert_eq!(
...
fn phase7_layered_request_accepts_pit_alpha_first_low_impact_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_es"));
    assert_eq!(
...
fn phase7_layered_request_accepts_pit_excess_return_recovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_et"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_bull_sleeve_cash_recovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_eu"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_return_first_fill_repair_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ey"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_pit_nonlinear_alpha_regime_rebuild_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ez"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_pit_quality_recovery_alpha_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fa"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_event_post_return_curve_alpha_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fb"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_event_reaction_alpha_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fc"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_broad_financial_feature_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fg"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_broad_financial_stratified_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fh"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_native_alpha_fusion_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fj"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_trainable_alpha_admission_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ft"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
    ...
    assert!(bundle.plan.trials.iter().take(12).all(|trial| {
        ...
        quant_api::discovery::strategy_discovery::is_phase7_base_trainable_alpha(combo_name)
    }));
    assert!(!bundle.plan.trials.iter().any(|trial| {
        trial.parameters["combo_name"] == "phase7_event_surprise_v1"
            ...
            || trial.parameters["combo_name"] == "phase7_quality_event_reaction_segments_overlay_v1"
    }));
}
...
fn phase7_layered_request_accepts_v19_shareholder_structure_sleeve_profile() {
    ...
    assert!(bundle.plan.trials.iter().take(12).any(|trial| {
        trial.parameters["combo_name"] == "phase7_fq_change_shareholder_structure_sleeve_10pct_v1"
...
fn phase7_layered_request_accepts_v19_train_window_ml_h120_low_impact_rebuild_profile() {
    ...
    assert_eq!(
        phase7_train_window_ml_feature_profile_for_search(Some("phase7_v19_ml_h120_low_impact",)),
        "phase7_gb_quality_value_recovery_low_impact_v6"
    );
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some(
        "phase7_v19_ml_h120_low_impact",
...
fn phase7_layered_request_accepts_v19_train_window_ml_event_sentiment_rebuild_profile() {
    ...
    assert_eq!(
        phase7_train_window_ml_feature_profile_for_search(Some("phase7_v19_ml_event_sentiment",)),
...
fn phase7_layered_request_accepts_prediction_capacity_dual_objective_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fl"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_prediction_target_gross_signal_fidelity_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fm"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_prediction_confidence_turnover_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fn"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
    ...
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["prediction_confidence_turnover_profile"] == "return_anchor_confidence_off"
            && trial.parameters["rebalance_hysteresis_pct"] == "0"
            && trial.parameters["partial_rebalance_ratio"] == "1"
            && trial.parameters["execution_impact_budget"] == "off"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["prediction_confidence_turnover_profile"] == "confidence30_turnover_smooth"
            && trial.parameters["prediction_min_percentile"] == "0.30"
            && trial.parameters["rebalance"] == "160"
            && trial.parameters["rebalance_hysteresis_pct"] == "0.01"
            && trial.parameters["partial_rebalance_ratio"] == "0.75"
...
fn phase7_layered_request_accepts_prediction_confidence_alpha_lift_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fo"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_prediction_long_horizon_low_turnover_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fp"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_prediction_long_horizon_regime_alpha_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fr"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_h60_nonlinear_stress_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fw"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_h120_low_impact_stress_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fz"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_train_window_nonlinear_ranking_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fx"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_train_window_stress_fill_target_exposure_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ga"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_train_window_ml_stress_fill_discovery_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_gb"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "prediction_confidence_stress_fill_quality_score_v1"
    );
    ...
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["train_window_ml_ranking_profile"] == "nlq_ranker_rae_h45_bucket7_fill95"
            && trial.parameters["capacity_risk_budget"]
                == "capacity_stress_participation_blended_alpha_headroom_floor_70_v1"
    }));
}
...
fn phase7_layered_request_accepts_current_event_nonlinear_alpha_discovery_profile() {
    ...
    assert_eq!(bundle.plan.planned_trials, 8);
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_fs"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_oos_regime_alpha_rebuild_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ev"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_oos_benchmark_excess_rebuild_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ew"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn phase7_layered_request_accepts_oos_execution_adaptive_rebuild_profile() {
    ...
    let gate_policy = default_oos_train_selection_gate_policy_for_search_profile(Some("phase7_ex"));
    assert_eq!(
        gate_policy["train_stress_score_profile"],
        "capacity_stress_return_score_v1"
    );
...
fn sleeve_admission_diagnostic_matrix_explains_rejected_family_by_window() {
    ...
    let report =
        sleeve_admission_diagnostic_matrix_json("exp-sleeve", "completed", &metrics, &rows_by_task);
...
fn phase7_layered_request_accepts_return_distribution_repair_profile() {
    ...
    assert!(bundle.plan.trials.iter().any(|trial| {
        trial.parameters["market_regime"] == "quality_event_window_return_sharpe_router_v3"
            ...
            && trial.parameters["event_gate_combo_name"] == "phase7_event_window_earnings_10d_v1"
    }));
    assert!(bundle.plan.trials.iter().any(|trial| {
...
fn phase7_layered_request_accepts_volatility_sharpe_profile() {
    ...
    assert!(bundle
        ...
        .trials
        .iter()
        .any(|trial| { trial.parameters["portfolio_volatility_control"] == "vol120_20_60_100" }));
    assert!(bundle
        .plan
        .trials
        .iter()
        .any(|trial| { trial.parameters["portfolio_volatility_control"] == "vol120_18_55_100" }));
}
...
fn phase7_layered_request_accepts_regime_position_sharpe_profile() {
    ...
    assert!(bundle
        ...
        .trials
        .iter()
        .any(|trial| { trial.parameters["market_regime"] == "quality_bear_position_guard_v1" }));
    assert!(bundle
        .plan
        .trials
        .iter()
        .any(|trial| { trial.parameters["market_regime"] == "quality_bear_position_guard_v2" }));
}
        ...
        oos_output: FactorBacktestRunOutput {
            ...
            market_feature_prewarm_report: Some(
                quant_backtest::signal_generator::MarketFeaturePrewarmReport {
                    snapshot_key: quant_backtest::signal_generator::MarketFeatureSnapshotKey::new(
                        "full-market-2016-v1",
                        NaiveDate::from_ymd_opt(2020, 1, 1).unwrap(),
...
fn candidate_input_uses_best_trial_when_trial_id_is_blank() {
    ...
    let normalized =
        normalize_promote_request("trial-opt-demo-0001", req).expect("normalized promote request");
...
fn elite_robustness_gate_rejects_low_calmar_and_profit_factor() {
    ...
    assert!(gates
        .iter()
        .any(|gate| { gate["gate"] == "max_drawdown_duration_days" && gate["passed"] == false }));
}
...
fn robustness_gate_rejects_overfit_candidate_with_weak_window_distribution() {
    ...
    assert!(gates
        .iter()
        .any(|gate| { gate["gate"] == "walk_forward_median_sharpe" && gate["passed"] == false }));
    assert!(gates
        .iter()
