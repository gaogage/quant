...
fn data_readiness_strict_gate_blocks_required_yellow() {
    ...
    let blocked = data_readiness_blocking_checks(&checks, DataReadinessGate::BlockRequiredYellow);
...
fn phase7_new_alpha_candidate_sources_mark_p315_boundaries() {
    ...
    assert_eq!(
        by_source["industry_prosperity_proxy"]["alpha_admission_gate"]["required_universe_profile"],
        "main_chinext_non_st"
    );
    assert_eq!(
...
fn phase7_p319_candidate_admission_tracks_new_sources_and_stop_families() {
    ...
    assert!(admission["stopped_same_family_sources"]
        ...
        .unwrap()
        .iter()
        .any(|source| source == "multi_vendor_analyst_revision_current_akshare_cninfo_revision"));
...
fn share_float_readiness_requires_requested_float_date_span() {
    ...
    let smoke_only = phase7_share_float_feature_readiness(10, smoke_covered_days, expected_days, 0);
...
fn akshare_analyst_revision_raw_row_blocks_publication_date_mismatch() {
    ...
    let error =
        akshare_analyst_revision_raw_row_from_record(&record, "20260624", &open_dates).unwrap_err();
...
fn structured_order_capacity_price_chain_source_contract_requires_structured_event_schema_and_precision_gates(
    ...
    assert!(contract["coverage_audit_required"]
        ...
        .iter()
        .any(|gate| {
            gate == "correlation_vs_existing_moneyflow_liquidity_price_volume_quality_event_sources"
        }));
}
...
fn structured_order_capacity_price_chain_source_evidence_inventory_records_real_candidates() {
    ...
    for candidate in candidates {
        ...
        assert!(
            candidate["missing_required_evidence"]
                .as_array()
                .unwrap()
                .iter()
                .any(|item| item
...
fn structured_order_capacity_price_chain_cninfo_operator_evidence_contract_defines_external_manifest_without_secrets(
    ...
    let contract = phase7_structured_order_capacity_price_chain_cninfo_operator_evidence_contract();
...
fn structured_order_capacity_price_chain_cninfo_permission_sample_smoke_plan_requires_manifest_audit_pass(
    ...
    let plan = phase7_structured_order_capacity_price_chain_cninfo_permission_sample_smoke_plan();
...
fn exchange_announcement_order_capacity_manual_schema_review_allows_only_bounded_sync_design() {
    ...
    assert!(review["ddl_checks"]["required_fields"]
        ...
        .iter()
        .any(|field| field == "source_published_at_quality"));
    assert!(review["next_required_steps"]
        .as_array()
        .unwrap()
        .iter()
        .any(|step| step == "build_plan_only_calendar_day_symbol_category_bounded_sync_design"));
}
...
fn exchange_announcement_order_capacity_coverage_quality_contract_blocks_training_until_small_batch_audits_pass(
    ...
    let contract = phase7_exchange_announcement_order_capacity_coverage_quality_audit_contract();
...
fn exchange_announcement_order_capacity_probe_normalizes_only_akshare_empty_dataframe_key_error() {
    ...
    let unsupported =
        normalize_exchange_announcement_order_capacity_probe_payload(unsupported_category_payload);
...
fn exchange_announcement_order_capacity_coverage_quality_keeps_excluded_category_parser_failures_non_blocking(
    ...
    assert_eq!(
        passed_with_excluded_failure["summary"]["excluded_unsupported_category_failed_attempts"],
        json!(4)
    );
...
fn exchange_announcement_order_capacity_manual_precision_sample_report_requires_human_review_before_p310(
    ...
    let undersized =
        exchange_announcement_order_capacity_manual_precision_sample_report(20, 50, 20, 3, vec![]);
    assert_eq!(
        undersized["status"],
...
fn exchange_announcement_order_capacity_pdf_detail_audit_decision_requires_hash_availability_and_spans(
    ...
    let passed = decide_exchange_announcement_order_capacity_pdf_detail_audit(2, 2, 2, 2, 2, 0, 0);
    assert_eq!(
        passed["admission_decision"],
    ...
    let scanned = decide_exchange_announcement_order_capacity_pdf_detail_audit(2, 2, 2, 2, 2, 1, 0);
    assert_eq!(
...
fn shareholder_structure_raw_schema_lands_period_anomalies_for_audit() {
    ...
    assert!(ddl.contains("DROP CONSTRAINT IF EXISTS market_stock_holder_number_period_pit_check"));
...
fn shareholder_structure_coverage_decision_requires_breadth_pit_and_duplicates() {
    ...
    assert_eq!(
        decide_shareholder_structure_coverage_audit(true, 0, 0.0, 0, 0, 0, 0)["admission_decision"],
        "bounded_sync_required_before_coverage_audit"
    );
    assert_eq!(
...
fn futures_price_chain_coverage_audit_sql_breaks_down_year_endpoint_product_exchange() {
    ...
    assert!(coverage_sql.contains("GROUP BY endpoint, trade_year, product_symbol, exchange_key"));
...
fn futures_price_chain_mapping_candidate_validation_rejects_invalid_target_and_empty_evidence() {
    ...
    let result =
        validate_futures_price_chain_mapping_candidate(&candidate, &raw_products, &sw2021_targets);
