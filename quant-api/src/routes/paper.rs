...
fn default_account_type() -> String { "simulated".to_string() }
...
async fn simulate_paper_nav_inner(
    ...
    .bind(&account_id)
    .fetch_optional(db).await.map_err(|e| format!("DB: {}", e))?
    .ok_or("account not found")?;
    let initial = row.0;
    ...
    .bind(&pred_id).bind(start).bind(end)
    .fetch_all(db).await.map_err(|e| format!("scores: {}", e))?;
    if scores.is_empty() { return Err("no prediction scores found".into()); }
    ...
    .fetch_all(db).await.map_err(|e| format!("days: {}", e))?;
    ...
    .fetch_all(db).await.map_err(|e| format!("prices: {}", e))?;
    ...
    for day_idx in 0..trading_days.len() {
        ...
        if day_idx >= next_reb_idx {
            let score_day = scores_by_date.keys()
                .filter(|&&d| d <= today)
                .max().copied();
                    ...
                    if candidates.len() >= 5 {
                        ...
                        for (sym, target_val) in &target_values {
                            if let Some(px) = price_map.get(&(today, sym.clone())) {
                                ...
                                if diff.abs() * px > 100.0 { // min trade size
                                    let cost = diff.abs() * px * (1.0 + if diff > 0.0 { commission } else { 0.0 });
        ...
        let daily_ret = if prev_nav > 0.0 { nav / prev_nav - 1.0 } else { 0.0 };
        let cum_ret = if initial > 0.0 { nav / initial - 1.0 } else { 0.0 };
        if nav > peak_nav { peak_nav = nav; }
        let dd = if peak_nav > 0.0 { (peak_nav - nav) / peak_nav } else { 0.0 };
        if dd > max_dd { max_dd = dd; }
    ...
    .bind(prev_nav).bind(peak_nav).bind(max_dd).bind(total_trades as i32).bind(&account_id)
    .execute(db).await.map_err(|e| format!("account update: {}", e))?;

    let sharpe = nav_history.iter()
        .map(|v| v["daily_return"].as_f64().unwrap_or(0.0))
        .collect::<Vec<_>>();
    let avg_ret = sharpe.iter().sum::<f64>() / sharpe.len().max(1) as f64;
    let var = sharpe.iter().map(|r| (r - avg_ret).powi(2)).sum::<f64>() / sharpe.len().max(1) as f64;
    let annual_sharpe = if var > 1e-12 { avg_ret / var.sqrt() * (252.0f64).sqrt() } else { 0.0 };
...
pub struct HistoricalReplayRequest {
    ...
    #[serde(default)]
    pub cov_method: Option<String>, // 协方差估计方法: "linear" (默认) | "nonlinear" | "rmt"
}
...
fn normalize_account_request(
    ...
    Ok(NormalizedAccountRequest {
        ...
        base_currency: req
            ...
            .unwrap_or("CNY")
            .to_string(),
        operator: req.operator.as_deref().map(str::trim).filter(|v| !v.is_empty()).map(str::to_string),
        account_type: req.account_type.trim().to_string(),
        dingtalk_webhook_url: req.dingtalk_webhook_url.as_deref().map(str::trim).filter(|v| !v.is_empty()).map(str::to_string),
    })
}

/// POST /api/v1/quant/paper/accounts/{account_id}/notify-position
///
/// Pushes current position summary to DingTalk for the given paper account.
/// Requires a `dingtalk_webhook_url` on the account.
pub async fn notify_position(
    State(state): State<Arc<AppState>>,
    Path(account_id): Path<String>,
) -> impl IntoResponse {
    match push_position_to_dingtalk(&state.db, &account_id).await {
        Ok(summary) => Json(json!({"code": 0, "data": summary})),
        Err(message) => Json(json!({"code": 1, "message": message})),
    }
}

async fn push_position_to_dingtalk(
    db: &sqlx::PgPool,
    account_id: &str,
) -> Result<Value, String> {
    use super::dingtalk;
    ...
    // Load account info
    let account = sqlx::query_as::<_, (String, String, Option<String>, Option<f64>)>(
        "SELECT name, account_type, dingtalk_webhook_url, current_nav::double precision
    ...
    // Resolve webhook URL: account-level config first, then .env DINGTALK_CLIENT_ID
    let webhook_url = match &account.2 {
        Some(url) if !url.is_empty() => url.clone(),
        _ => dingtalk::build_dingtalk_webhook_url()
            .ok_or_else(|| "No DingTalk webhook URL: configure account dingtalk_webhook_url or set DINGTALK_CLIENT_ID in .env".to_string())?,
    };
    ...
    .fetch_all(db)
    .await
    .map_err(|e| format!("Position query failed: {}", e))?;

    let positions: Vec<Value> = pos_rows
        .iter()
        .filter_map(|(sym, qty, price, name)| {
            let q = qty.unwrap_or(0.0);
            let p = price.unwrap_or(0.0);
            if q <= 0.0 { return None; }
            Some(json!({
                "symbol": sym,
                "name": name.as_deref().unwrap_or(sym),
                "quantity": q,
                "current_price": p,
                "market_value": q * p,
            }))
    ...
    let class_rows = sqlx::query_as::<_, (String, Option<f64>)>(
        "SELECT CASE
                  ...
                  ELSE CONCAT('A股-', COALESCE(NULLIF(ms.market,''), NULLIF(ms.exchange,''), '其他')) END,
                SUM(pp.quantity * COALESCE(pp.market_price, pp.avg_cost))::double precision
         FROM paper_position pp
         LEFT JOIN market_stock ms ON ms.symbol = pp.symbol
         WHERE pp.paper_account_id = $1 AND pp.quantity > 0
         GROUP BY 1
         ORDER BY SUM(pp.quantity * COALESCE(pp.market_price, pp.avg_cost)) DESC",
    )
    .bind(account_id)
    .fetch_all(db)
    .await
    ...
    let class_breakdown: Vec<Value> = class_rows.iter().map(|(cls, v)| {
    ...
    let total_nav = account.3.unwrap_or(0.0);
    let market_value: f64 = positions.iter()
        .filter_map(|p| p.get("market_value").and_then(|v| v.as_f64()))
        .sum();
    let cash = total_nav - market_value;
    let initial_capital_row = sqlx::query_as::<_, (Option<f64>, Option<f64>)>(
        "SELECT initial_capital::double precision, max_drawdown_pct::double precision
    ...
    .await
    .map_err(|e| format!("Query failed: {}", e))?
    .unwrap_or((Some(total_nav), Some(0.0)));

    let initial = initial_capital_row.0.unwrap_or(total_nav);
    let cumulative_return = if initial > 0.0 { (total_nav - initial) / initial } else { 0.0 };
    let max_drawdown = initial_capital_row.1.unwrap_or(0.0);
    ...
    dingtalk::send_dingtalk_markdown(&webhook_url, "持仓摘要", &text).await?;
    ...
    Ok(json!({
        ...
        "cumulative_return_pct": (cumulative_return * 100.0 * 100.0).round() / 100.0,
        "max_drawdown_pct": (max_drawdown * 100.0 * 100.0).round() / 100.0,
