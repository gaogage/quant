...
## 部署窗口纪律（2026-09-18 用户定版）

**交易时段不动生产**；以下三个窗口**禁止安排任何定时任务**，留给系统重新部署或 Mac 关机：
- 凌晨 00:00–08:00
- 中午 11:30–13:00
- 晚上 17:00–19:00

新增定时任务（scheduled_task_config 或 run_tick 硬编码）必须先核对窗口表；现有任务分布见 `docs/projects/quant/knowledge/quant架构评估与DDD重构路线-2026-09-18.md` §四。

## 绩效口径纪律（2026-09-19 用户定版）

- **策略年化收益率永远用几何 CAGR**（`(1+累计)^(252/n)−1`），禁止算术外推 `(1+日均)^252`（波动虚高，实测 6.8 年 +2.6pp 误报）。对外报告一律几何口径。
- **模拟盘绩效自含全成本**：佣金/印花税经 `ExecutionFeeSchedule`（trading.rs）在模拟成交时真扣（佣金万 2.5 最低 5 元、印花税万 2.5 卖出单边，env `PAPER_FEE_*` 可覆盖）；滑点内含于成交价。费率属执行器/券商渠道配置，多渠道时扩展为账户级。
